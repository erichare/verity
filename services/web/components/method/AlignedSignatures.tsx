...
function SignatureChart({
  ...
  const xs = (px: number) => (px / maxLen) * width;
  ...
  const draw = (delay: number) => ({
    strokeDasharray: 1,
    strokeDashoffset: reducedMotion || shown ? 0 : 1,
    transition: reducedMotion
      ? "none"
      : `stroke-dashoffset 1.1s cubic-bezier(0.4, 0, 0.2, 1) ${delay}s`,
  });
  const fade = (delay: number) => ({
    opacity: reducedMotion || shown ? 1 : 0,
    transition: reducedMotion ? "none" : `opacity 0.5s ease ${delay}s`,
  });
  const m = Math.min(bandsA.length, bandsB.length);
  const labelSize = mobile ? 16 : 11;
  const labelOffset = mobile ? 18 : 12;
  ...
  return (
    ...
    >
      {bandsA.map((r, i) => (
        <rect
          key={`a${i}`}
          x={xs(r.x_frac * a.length)}
          y={aY0}
          width={xs(r.w_frac * a.length)}
          height={laneH}
          fill="var(--brass)"
          fillOpacity={0.16}
          style={fade(0.7)}
      ...
      {bandsB.map((r, i) => (
        <rect
          key={`b${i}`}
          x={xs(r.x_frac * b.length)}
          y={bY0}
          width={xs(r.w_frac * b.length)}
          height={laneH}
          fill="var(--brass)"
          fillOpacity={0.16}
          style={fade(0.7)}
      ...
      <path
        ...
        style={draw(0.35)}
      />
      <text x={6} y={aY0 + labelOffset} fontSize={labelSize} fill="var(--muted)">
        Mark A
      </text>
      <text x={6} y={bY0 + labelOffset} fontSize={labelSize} fill="var(--muted)">
        Mark B
      </text>
    </svg>
  );
