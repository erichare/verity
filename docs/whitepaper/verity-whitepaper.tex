...
per study and never pool across firearm makes.
\end{abstract}
...
\begin{enumerate}
  \item \textbf{Unification.} One calibrated pipeline scores striated, impressed,
        and fractured marks through \emph{Congruent Matching Regions}
...
manufactured cartridge-case slides (breech faces)~\cite{fadul2011}. Consecutively
...
\begin{itemize}
  ...
  \item \textbf{The learned representation does not yet beat the baseline.} A
        ...
        success, and the deployed system uses the interpretable baseline.
  \item \textbf{Impressed-mark validation is earlier-stage} than the striated path:
        Fadul gives a credible demonstration on consecutively manufactured breech
        faces, but with far fewer sources than the bullet benchmarks.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{A local-first data catalog.} A catalog service harvests and labels
        the open NBTRD~\cite{nbtrd2020} 3-D scans (which have no public API) and
        other open sets, so validation runs against versioned, reproducible data.
  \item \textbf{Reproducible validation reports.} The numbers in
        \S\ref{sec:validation} come from a validation harness over
        \code{data/ablation.json}; the figures in this paper are regenerated from
        that file by a checked-in script.
\end{itemize}
