...
$\Cllr = \num{0.113} \pm \num{0.066}$ with test AUC $\num{1.000}$; we report results
per study and never pool across firearm makes. Beyond bullets, the \emph{same}
pipeline under \emph{one} scorer configuration is validated source-disjoint on
impressed cartridge breech faces (Fadul) and striated screwdriver toolmarks
(\code{tmaRks}), where CMR recovers Song's Congruent Matching Cells and the Chumbley
statistic respectively --- one calibrated algorithm, three reductions, each
\emph{demonstrated} rather than asserted.
\end{abstract}
...
The automatic comparison of bullet land impressions by cross-correlation of
extracted striation signatures is the CSAFE / Iowa State method of Hare, Hofmann
\& Carriquiry~\cite{hare2017}, with the open \code{bulletxtrctr} pipeline and the
three-similarity-score study of Vanderplas et al.~\cite{vanderplas2020} as the
reference implementation and benchmark. The cell-counting idea for impressed marks
...
it~\cite{song2018}. The objective comparison of striated toolmarks by a
distribution-free statistic is Chumbley et al.~\cite{chumbley2010}. The surface
...
Br\"ummer \& du Preez~\cite{brummer2006}.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}llll@{}}
    \toprule
    \textbf{Modality} & \textbf{Region} & \textbf{Transform group $G$} & \textbf{Recovers} \\
    \midrule
  ...
  \caption{Congruent Matching Regions instantiated per modality. The 2-D row
    \emph{is} CMC; the 1-D row \emph{is} the lag-agreement (consecutive-matching-striae)
    statistic. One implementation, three marks.}
  \label{tab:cmr}
\end{table}
...
The default calibration is a two-parameter logistic fit (Platt scaling) of the
same-source posterior on the score. Two deliberate settings make it the right
...
\paragraph{The ELUB bound (the firewall's safety rail).}
...
catastrophically wrong, large-magnitude LR that dominates $\Cllr$. Following the
empirical-bound (ELUB) principle~\cite{vergeer2016}, Verity caps $|\log_{10}
\mathrm{LR}|$ at what the data can support. The default cap is set from the rarer
class count,
\begin{equation}
...
$n_{\text{same}}\!:\!1$ from $n_{\text{same}}$ same-source examples. The cap is
...
automatically. Barrel-disjoint, the bound roughly halves the calibration loss
...
rate (EER), and Tippett plots.

\paragraph{A credible interval on the LR.}
Because the score$\to$LR map is itself estimated from a finite reference, a point LR
hides its own uncertainty. Verity reports a \emph{percentile credible interval on
$\log_{10}\mathrm{LR}$} by resampling the reference and \emph{refitting the same
bounded calibration in every replicate} --- so the firewall holds per draw and the
interval inherits the monotone, bounded property. Two schemes are provided:
\emph{row-stratified} (resample within each class, holding the class sizes and hence
the ELUB bound fixed, isolating calibration noise) and \emph{clustered / block}
...
asymmetric and piles at the ELUB cap; the reported point is the full-reference LR,
...
(currently: striated bullet lands, and impressed breech-face marks) and declines to
...
that manufactures a misleading single error rate. Reported $\Cllr$ values are the
...
consecutively-manufactured slotted-screwdriver set (580 marks across 56 tool edges),
a non-firearm domain the pipeline was never tuned for. Consecutively
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lll@{}}
    ...
    ELUB bound          & $|\log_{10}\mathrm{LR}|\approx \num{1.66}$ & Eq.~\ref{eq:elub} \\
    \midrule
    \multicolumn{3}{@{}l}{\emph{Barrel-disjoint test (10 barrels, 46 KM / 549 KNM)}}\\
...
areal cross-correlation (pooled AUC \num{0.911}, $\Cllr\,\num{0.529}$) almost all the way
to the \code{cmcR} Congruent Matching Cells specialist (AUC \num{1.000},
$\Cllr\,\num{0.194}$) --- with no cartridge-specific engineering. CMR-2D \emph{is} CMC,
...
\code{tmaRks} consecutively-manufactured screwdriver set (580 marks across 56 tool
edges), source-disjoint by the mark-generating edge. CMR-1D reaches AUC \num{0.957} at
...
evidence the unification is not firearm-specific. (On the much smaller Ames Lab set the
Chumbley \code{toolmaRk} $U$ specialist is itself weak --- pooled AUC \num{0.614} over 7
tools --- a known small-sample effect, which is why we report \code{tmaRks} as the
primary toolmark benchmark.)
...
source-disjoint, recovers the field-standard method in each of the three reductions of
Table~\ref{tab:cmr}} --- the Chumbley statistic on toolmarks, the CMC count on breech
faces, and the bullet-land result --- rather than three bespoke pipelines.
...
\begin{itemize}
  \item \textbf{Small source-disjoint sets widen uncertainty.} Consecutively
        ...
        (Fig.~\ref{fig:cllr}), and the ELUB bound (Eq.~\ref{eq:elub}) is
        correspondingly conservative. More independent sources would tighten both.
  ...
  \item \textbf{The cross-modality reductions rest on few sources.} The impressed
        ...
        sources than the bullet benchmarks, so the wider $\Cllr$ and conservative ELUB
        bounds reflect that. They are a genuine cross-modality demonstration of the CMR
...
\begin{itemize}
  ...
  \item \textbf{A calibrated-LR REST API.} A deployed service exposes \code{/detect}
        ...
        bounded LR with its credible interval and the region attribution), and a
  ...
  \item \textbf{A local-first data catalog.} A catalog service harvests and labels
        the open NBTRD~\cite{nbtrd2020} 3-D scans --- which have no public API --- the
        first open programmatic harvester for that benchmark (the prior CSAFE scraper
        has been unmaintained since 2020), alongside the Fadul and \code{tmaRks} sets,
        so validation runs against versioned, content-addressed, reproducible data.
