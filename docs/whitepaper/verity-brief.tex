...
\begin{itemize}
  ...
  \item \textbf{Characterize it honestly.} Quote the calibration cost $\Cllr$
        ...
        both training and test), reported \emph{per study}, never pooled across makes.
...
regions whose best-fit transforms agree. A 1-D window under lag recovers the
Chumbley striae statistic; a 2-D cell under translation+rotation \emph{is} CMC; a
...
reference supports, with a bootstrap credible interval) --- so the evidence is
...
test AUC $= \num{1.000}\pm\num{0.001}$}\\[1pt]
{\small (46 same-source / 549 different-source comparisons over 10 held-out barrels;
in-sample $\Cllr = \num{0.077}$, $\Cllr^{\min}=\num{0.032}$, ECE $=\num{0.031}$,
EER $=\SI{2.1}{\percent}$.)}
\end{claimbox}
