...
{\footnotesize\color{vgray}\textbf{Independent, and not claiming otherwise.}
Verity is an independent project built on published, public research it did not
invent. It is not affiliated with, endorsed by, or developed in partnership with
CSAFE, NIST, or Iowa State University; mention of that work denotes intellectual
lineage only, and building on public research implies nothing about its authors'
or their institutions' views of Verity. The next step is validation on larger,
representative, genuinely source-disjoint references; the codec, the calibration
firewall, and the validation harness are offered as open infrastructure to anyone
producing such data.}
